...
def shear(img, sv, sh):
    ...
    x_shape = int(img.shape[0]*(1 + sv)) #Scale x-axis in new image to fit all new coordinates
    y_shape = int(img.shape[1]*(1 + sh)) #Scale y-axis in new image to fit all new coordinates
...
ax[0,1].set_title('Img scaled by a factor of 0.5')
#Subtask b
ax[1,0].imshow(rotate(img, np.pi/3), cmap='gray')
ax[1,0].set_title('Img rotated by 60 degrees')
#Subtask c
ax[1,1].imshow(translate(img, 50, 100), cmap='gray')
ax[1,1].set_title('Translated Image')
#Subtask d
ax[2,0].imshow(shear(img, 0.5, 0), cmap='gray')
ax[2,0].set_title('Verticaly sheared image')

ax[2,1].imshow(shear(img, 0, 0.5), cmap='gray')
ax[2,1].set_title('Horizontaly sheared image')
