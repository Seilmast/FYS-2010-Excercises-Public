...
some_range = np.linspace(-24, 152, num=100)
print(f'Initial range: {np.min(some_range)} to {np.max(some_range)}')
some_new_range = new_range(some_range, min_val=0, max_val = 255)
print(f'Final range: {np.min(some_new_range)} to {np.max(some_new_range)}')